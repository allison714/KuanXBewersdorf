...
% % === Sigma Display ===
% uicontrol(f, 'Style', 'text', 'Position', [20 ypos(13) 180 20], 'String', 'sigmaXY (px):');
% sigmaXYText = uicontrol(f, 'Style', 'text', 'Position', [200 ypos(13) 80 20], 'String', '');
% uicontrol(f, 'Style', 'text', 'Position', [300 ypos(13) 180 20], 'String', 'sigmaZ (px):');
% sigmaZText = uicontrol(f, 'Style', 'text', 'Position', [480 ypos(13) 60 20], 'String', '');
...
% onGenerate function
    function onGenerate()
        ...
        % Load the stack and apply Gaussian filter to each slice (integer)
        stack = [];
        ...
        function imgOut = addScale(img)
            scaleBar_um = 1;  % 1 um scale bar
            scaleBar_px = round(scaleBar_um / voxelX);
            posX = size(img,2) - scaleBar_px - 40;
            posY = size(img,1) - 60;
            % U
            % Adjust font size for better legibility
            % fontSize = 14;

            imgOut = insertShape(toGreen(img), 'FilledRectangle', ...
                [posX posY scaleBar_px 5], 'Color','white', 'Opacity',1);
            % imgOut = insertText(imgOut, [posX, posY - 25], sprintf('%.1f µm', scaleBar_um), ...
            %     'FontSize', fontSize, 'Font', 'Arial', 'BoxOpacity', 0, 'TextColor', 'white', ...
        ...
        % Save the non-cropped panels figure
